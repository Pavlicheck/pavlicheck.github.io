...
let actions = {
	...
	hole: function	() {
		...
		clearInterval(gameInterval);
		safeData();
		$('.play-state').css('display', 'flex').animate({opacity: 1}, 500).delay(500).append('<h1>Game over</h1><span>Result: ' + gameScore + '</span><button id="retry">Retry</button><button id="menu">Exit</button>');
		$('#retry').mousedown(() => { createField(8,8); $('.play-state').css({display: 'none', opacity: 0}).empty();});
		$('#menu').mousedown(() => exit());
